...
defineProps<{
  sectionTitle: string,
  section: Section,
  leftColumnIds: Array<string>,
  rightColumnIds?: Array<string>
}>();
</script>
...
<script lang="ts">
  export default {
    computed: {
      leftColumn: function() {
        return this.section.contents?.filter(sc => this.leftColumnIds.includes(sc.id));
      },
      rightColumn: function() {
        return this.rightColumnIds
        ? this.section.contents?.filter(sc => this.rightColumnIds?.includes(sc.id))
        : this.section.contents?.filter(sc => !this.leftColumnIds.includes(sc.id));
      }
    }
  }
</script>